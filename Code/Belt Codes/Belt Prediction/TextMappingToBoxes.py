        ...
        for text_element in texts_data:
            ...
            for detection in detections:
                ...
                if self.is_inside(text_bbox, product_bbox):
                    ...
                    track_ids_mapped.add(detection['TrackID'])
                    break

        # Add unmapped product detections
        for detection in detections:
            if detection['TrackID'] not in track_ids_mapped:
                mapped_data.append({'TrackID': detection['TrackID'], 'Text': '', 'Area_Ratio_Per_Character': 0})
