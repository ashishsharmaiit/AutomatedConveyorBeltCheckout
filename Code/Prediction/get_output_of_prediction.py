...
def get_top_words(combined_data, top_n=6):
    """
...
def find_product_name(top_words):
    """
    ...
    for box_id, words in top_words.items():
        ...
        for word_info in words:
            text = word_info['Text']  # Normalize the text
                ...
                for side in doc['Sides']:
                    for text_entry in side['Texts']:
                        if text_entry['Text'] == text:
                            product_scores[mongodb_box_id] += text_entry['Area_per_character']
                            break
        # If we found matching products, find the one with the highest score
        if product_scores:
            best_match = max(product_scores, key=product_scores.get)
            product_mapping[box_id] = best_match
        else:
            product_mapping[box_id] = None  # or some placeholder to indicate no match was found

    return product_mapping

# Main execution
if __name__ == '__main__':
    image_path = 'IMG_7155.JPG' 
    ...
    product_mapping = find_product_name(top_words)

    # Output the mapping of current product BoxID to MongoDB BoxID
